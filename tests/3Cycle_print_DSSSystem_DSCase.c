...
//        DSGMASystem *gma = NULL;
        ...
        DSCase *aCase = NULL;
        DSUInteger * termSignature, caseNumber = 27;
...
systemEquations = dsCyclicalCaseEquationsSplitVariables(aCase,
       ...
       for (int y=0; y<3; y++)
       {
        ...
        if (aCase->U == NULL){
        printf("Matrix U is NULL");
        }

        printf("The condition Matrix U is: \n");
        printMatrix(aCase->U);

        printf("The condition Matrix delta is: \n");
        printMatrix(aCase->delta);

        printf("The condition Matrix zeta is: \n");
        printMatrix(aCase->zeta);

        printf("Now let's take a look at the matrices alpha of the ssys: \n");
        printMatrix(aCase->ssys->alpha);

        printf("Now let's take a look at the matrices beta of the ssys: \n");
        printMatrix(aCase->ssys->beta);

        printf("Now let's take a look at the matrices Gd of the ssys: \n");
        printMatrix(aCase->ssys->Gd);

        printf("Now let's take a look at the matrices Gi of the ssys: \n");
        printMatrix(aCase->ssys->Gi);

        printf("Now let's take a look at the matrices M of the ssys: \n");
        printMatrix(aCase->ssys->M);

        printf("Now let's print Xd of ssys: \n ");
        printPool(aCase->ssys->Xd);

        printf("Now let's print Xd_a of ssys: \n ");
        printPool(aCase->ssys->Xd_a);

        printf("Now let's print Xd_t of ssys: \n ");
        printPool(aCase->ssys->Xd_t);

        printf("Now let's print Xi of ssys: \n ");
        printPool(aCase->ssys->Xi);

        // In the case of the S-system 27, the Matrix Ad is not invertible. Does it mean that
        // this case is null?
