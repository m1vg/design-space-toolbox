...
//        // Case 23
//        strings[0] = strdup("x1. =  + a11 + x4*k7 - x1*b11 - x1*k8 - 2*x2*b22 + 2*a31 - 2*x3*b33");
//        strings[1] = strdup("x2. = x1^2*b12-x2*b23");
//        strings[2] = strdup("x3. = x2*b23-x3*b31");
//        strings[3] = strdup("x4. =  + k1 + x1*k8 - x4*k7 - x4*k6");

        // Case 139
        strings[0] = strdup("x1. =  a11 + 2*b31*x3 - b11*x1 - 2*b12*(x1^2) + x4*k7 - k8*x1");
        strings[1] = strdup("x2. = b12*(x1^2) - b23*x2 - b22*x2");
        strings[2] = strdup("x3. = a31 + b23*x2 - b31*x3 - b33*x3");
        strings[3] = strdup("x4. =  k1 + k8*x1 - x4*k7 - k6*x4");
        ...
        DSDesignSpaceCalculateAllValidCases(ds);
        DSDesignSpaceCalculateAllValidCasesByResolvingCyclicalCases(ds);


        // the variable ds should contain a field called validcases. It is a dictionary. Let's try to
        // access the number of cases and list them.
        DSDictionary * valid_cases = ds->validCases;
        DSUInteger countprint = valid_cases->count;
        printf("The number of valid cases is: %u \n", countprint );

        // now let's try to list the names
        char * stringM = NULL;
        stringM = DSSecureCalloc(sizeof(char), 100);
        DSCase * aaCase;

        for (int i = 0; i < countprint; i++) {

                // get the first key and store it in stringM
                sprintf(stringM, "%d", valid_cases->names[i]);
                aaCase = DSDictionaryValueForName(valid_cases, stringM);
                if (aaCase==NULL){
                printf("case is null \n");
                }else{
              printf( "the signature of the case %s is: %s\n", valid_cases->names[i], aaCase ->signature);
                }
        ...
        // now that we have the keys of the dictionary, let's access fields like signature and caseIdentifier

        for (int i = 0; i < countprint; i++) {
                printf( "the first position of valid cases is:  %s \n", valid_cases->names[i] );
        }


//        // let's print the case Prefix of the internal design space located in case 27.
//        char * stringM = NULL;
//        stringM = DSSecureCalloc(sizeof(char), 100);
//        sprintf(stringM, "%d", 27);
////        printf("The case Prefix is: %s \n", ds->casePrefix);
//
////        DSCyclicalCase * dsintern = DSDictionaryValueForName(ds->cyclicalCases, stringM);
////        printf("The case Prefix is: %s \n", dsintern -> internalDesignspace ->casePrefix );
//
//
//        // Now let's generate a case.
//
        DSCase *aCase = NULL;
        DSUInteger * termSignature, caseNumber = 139; // old case number 27
...
//         create a new DS. Use condition Matrices from case27

//         first create ds2
//        DSDesignSpace * ds2;

//        ds2 = DSDesignSpaceByParsingStringsWithXi(extendedSystem,
//                                                  DSGMASystemXd_a(DSDesignSpaceGMASystem(ds)),
//                                                  DSGMASystemXi(DSDesignSpaceGMASystem(ds)),
//                                                  DSDesignSpaceNumberOfEquations(ds));

        // assign condition matrices from case 27
//       DSDesignSpaceAddConditions(ds2, DSCaseCd(aCase), DSCaseCi(aCase), DSCaseDelta(aCase));

//       DSDesignSpaceCalculateCyclicalCases(ds2);
//        printf("Number of valid cases of ds2 is: %i\n", DSDesignSpaceNumberOfValidCases(ds2));

        // print those cases.
//        char ** keys = ds2->validCases->names;
//        printf("the valid cases inside ds2 are: %s %s %s \n", *(*(keys)),  *(*(keys+1)), *(*(keys+2)) )   ; // *ds2->validCases->names[1], *ds2->validCases->names[2]
//        printf("the valid cases inside ds2 are: %u %u %u \n", *(*(keys) +1),  *(*(keys+1)+1), *(*(keys+2)+1) )   ;
//        printf("the valid cases inside ds2 are: %u %u %u \n", *(*(keys) +2),  *(*(keys+1) +2), *(*(keys+2)+2) )   ;


        // the case signature for the valid cases are:
//         sprintf(stringM, "%d", 3);
//        DSCase *validcase1 =  DSDictionaryValueForName(ds2->validCases, stringM);
//        DSUInteger * signature1 = validcase1->signature;
////        printf("the case signature for the valid cases are: %u  ", signature1[0] );
...
//        printf("The term signature used to generate the case was: %u %u %u %u %u %u \n", *(termSignature),
//        *(termSignature+1), *(termSignature+2), *(termSignature+3), *(termSignature+4), *(termSignature+5) );
...
systemEquations = dsCyclicalCaseEquationsSplitVariables(aCase,
       ...
       for (int y=0; y<4; y++)
       {
        ...
        printf("The number of primary variables after: %u \n", (*primaryVariables));
