...
extern void DSuIntegerMatrixSetValue(DSuIntegerMatrix *matrix, const DSUInteger row, const DSUInteger column, const DSUInteger value);
...
extern DSUInteger * DSMatrixSwapRows(DSMatrix *matrix, DSUInteger rowA, DSUInteger rowB);
extern DSMatrix * DSMatrixSwapRowsBack(DSMatrix *matrix, DSUInteger *swappingVector);
extern DSMatrixArray * DSMatrixGaussElimination(const DSMatrix *Ad, const DSMatrix *Ai, const DSMatrix *B);
...
#if defined(__APPLE__) && defined (__MACH__)
#pragma mark - Matrix QHull conversions
#endif
...
extern DSuIntegerMatrix * DSuIntegerMatrixDecode(size_t length, const void *);
